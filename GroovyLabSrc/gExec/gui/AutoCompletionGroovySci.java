...
// this class supports code completion operations
public class AutoCompletionGroovySci
{
    ...
    public AutoCompletionGroovySci()
    {
       ...
       Collections.sort(AutoCompletionGroovySci.scanMethodsGroovySci, new Comparator()  {
        ...
        // construct a view of the sorted scanMethodsGroovySci Vector as an array of Strings
        int countMethods = AutoCompletionGroovySci.scanMethodsGroovySci.size();
          
        listOfAllMethodsGroovySci = new String[countMethods];  // the array of strings available for completion
    ...
    public String[] getMatched(String prefix)
    {
        ...
        int i = firstIndexOfMatchedString(prefix);   // first matched entry
        int j = lastIndexOfMatchedString(prefix, i);  // last matched entry
        String[] matches = new String[j-i];

        for (int k = 0; k < matches.length; k++)  // get the matches
        {
    ...
    public String[] getMatchedRegEx(String prefix) 
     ...
     // find first index of a completion entry that matches with the prefix
     // performs a binary search at the sorted array of strings available for completion
