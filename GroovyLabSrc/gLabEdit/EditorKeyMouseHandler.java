   ...
   try {
    ...
    /*
      ...
      Option<EvaluationResult> result = null;
        ...
        //   GlobalValues.GroovyShell.setVariable(x.key(), x.value());
        if (result.isDefined())
           GlobalValues.consoleOutputWindow.append(result.get().toStringxx());
     ...
     if (GlobalValues.replCompleter ==null)                
         GlobalValues.replCompleter = new InstanceMemberCompleter(GlobalValues.globalJavaEvaluator);
     
     currentText = getSelectedTextOrCurrentLine();
    
       System.out.println(currentText);
         CompletionResult cc = null;
            try {
                 cc = GlobalValues.replCompleter.call(currentText);
            } catch (Exception ex) {
                Logger.getLogger(EditorKeyMouseHandler.class.getName()).log(Level.SEVERE, null, ex);
            }
        
   if (cc != null)
        System.out.println(cc.candidates().toString());
