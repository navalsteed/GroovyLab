...
public class ParallelMult {
    ...
    public static double [][]  pmul( final double [][] v1, final  double [][] v2)  {
     if (GlobalValues.hostIsLinux64 || GlobalValues.hostIsMac)  // fast BLAS works only for Linux
...
for (int  r=0; r < sN; r++)
  ...
  int   nthreads = Math.min(GlobalValues.nthreads, rN);  // larger number of threads than the number of cores of the system deteriorate performance
...
public final static double [][]  pmulblas(final double [][] v1, final double [][] v2) {
   ...
   int nthreads = Math.min(GlobalValues.nthreads,  Arows);
